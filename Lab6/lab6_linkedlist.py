...
class LinkedList:
    ...
    def __delitem__(self, i: int) -> None:
        ...
        # Remove the node after the node that cursor refers to.
        node = cursor.next
    ...
    def remove(self, x: any) -> None:
        ...
        """
        # Traverse the linked list until cursor refers to the
        # node before the node that contains x, or the tail node
        # if x isn't in the linked list.
        cursor = self._first
        while cursor.next is not None and cursor.next.item != x:
            cursor = cursor.next

        if cursor.next is None:
            # x wasn't found in the linked list.
            raise ValueError(
                'LinkedList: {0} is not in list'.format(repr(x)))

        # cursor.next is the node that contains x. Remove it from the
        # linked list by linking the node before it to the node after it.
        cursor.next = cursor.next.next

        if cursor.next is None:
            # We just removed the last node in the linked list.
            self._last = cursor
