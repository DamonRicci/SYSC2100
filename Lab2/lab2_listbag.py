...
class ListBag:
    ...
    def remove(self, item: any) -> any:
        ...
        if item not in self._elems:
            raise ValueError("bag.remove(x): " + str(item) + " not in bag")
    ...
    def grab(self) -> any:
        ...
        """
        if not self._elems:
            raise ValueError("Bag is empty, cannot grab an item")
    ...
    def __add__(self, other: 'ListBag') -> 'ListBag':
        ...
        new_list_bag = ListBag(self._elems)
        for item in other:
            new_list_bag.add(item)
        return new_list_bag
    ...
    def __eq__(self, other: 'ListBag') -> bool:
        ...
        if isinstance(other, ListBag):
            return set(self._elems) == set(other._elems)
        else:
            return False
