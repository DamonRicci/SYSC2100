...
#         return 1 + max(self._height(node.left), self._height(node.right))
   ...
   # Exercise 7
    ...
    def count(self, item: any) -> int:
        ...
        def _count(node: 'BinaryTree._Node', item: any) -> int:
            """Return the number of occurrences of item in the binary tree rooted at node."""
            if node is None:
                return 0
            count = int(node.x == item)
            count += _count(node.left, item)
            count += _count(node.right, item)
            return count

        return _count(self._root, item)
...
def build_binary_tree() -> BinaryTree:
    ...
    rootnode = BinaryTree._Node(1)
    tree._root = rootnode

    rootnode.left = BinaryTree._Node(2)  # Insert left child of root.
    rootnode.right = BinaryTree._Node(3)  # Insert right child of root.

    # Insert left child of left child of root.
    rootnode.left.left = BinaryTree._Node(4)
    # Insert right child of left child of root.
    rootnode.left.right = BinaryTree._Node(5)

    # Insert left child of right child of root.
    rootnode.right.left = BinaryTree._Node(6)
    # Insert right child of right child of root.
    rootnode.right.right = BinaryTree._Node(7)
...
def build_perfect_binary_tree(height: int) -> BinaryTree:
    """Build a perfect binary tree of the given height, and return the reference to the tree."""
    tree = BinaryTree()

    if height == 0:
        # Base case: create a single node with no children.
        tree._root = BinaryTree._Node(None)
    else:
        # Recursive case: create a new root node, and attach two perfect binary trees of height h-1 as its left and right children.
        rootnode = BinaryTree._Node(None)
        tree._root = rootnode

        left_subtree = build_perfect_binary_tree(height - 1)
        right_subtree = build_perfect_binary_tree(height - 1)

        rootnode.left = left_subtree._root
        rootnode.right = right_subtree._root
