...
class ArrayBag:
    ...
    def remove(self, item: any) -> any:
        ...
        for i in range(self._num_items):
            if self._elems[i] == item:
                # Shift all items after the found item one index to the left.
                for j in range(i, self._num_items - 1):
    ...
    def grab(self) -> any:
        ...
        elif self._num_items * 3 <= len(self._elems) * 2:
            self._elems = self._elems[:self._num_items * 2 // 3]
        index = random.randint(0, self._num_items - 1)
        item = self._elems[index]
        self._elems[index] = self._elems[self._num_items - 1]
        self._elems[self._num_items - 1] = None
        self._num_items -= 1
        return item
    ...
    def __add__(self, other: 'ArrayBag') -> 'ArrayBag':
        ...
        if not isinstance(other, ArrayBag):
            raise TypeError("'+' requires two ArrayBag objects")
