...
1. Using the split method while iterating over the lines. Ex: str.split()

2. By converting the list to a set, since a set cannot have duplicates.

3. Using the sort method on the list. Ex: str.sort()
...
def build_word_list(filename: str) -> list[str]:
    ...
    for line in infile:
        word_set = word_set.union(set(line.lower().translate(
